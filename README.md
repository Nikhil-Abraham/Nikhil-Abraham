# Nikhil Abraham

Engineer building software systems, tools, and infrastructure.  
Currently exploring backend architecture, distributed systems concepts, and developer tooling.

---

## Current Projects

- [**HopScotch**](https://github.com/Nikhil-Abraham/hopscotch) - TraceRoute visualizer
- [**TokenFlow**](https://github.com/Nikhil-Abraham/token-flow) — TypeScript rate limiting library (token bucket + Sliding Window Log algorithm)  
- [**ERP Systems**](https://github.com/Nikhil-Abraham/fuel-station-erp) — internal tooling and workflow automation experiments  
- [**DLM Systems**](https://github.com/Nikhil-Abraham/DLM_System) — Dynamic Load Management System for EV charging stations
- [**Duplicate Finder**](https://github.com/Nikhil-Abraham/duplicate_finder) — Tool to sort and check for duplicate media files
- [**MoveMouse**](https://github.com/Nikhil-Abraham/move-mouse) — lightweight utility that prevents system idle by periodically moving the cursor  


---

## Notes

Most repositories here are experimental projects, infrastructure tooling, or algorithm implementations built while learning and exploring system design concepts.
